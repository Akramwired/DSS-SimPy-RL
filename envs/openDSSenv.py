...
class openDSSenv(gym.Env):
    ...
    def __init__(self, _dss, _critical_loads, _line_faults,_switch_names,_capacitor_banks,debug=False,contingency=1,load_lb=0.8, load_ub=1.2,_ders=None):
        ...
        self.action_space = spaces.Discrete(n_actions - 1)
    ...
    def step(self, action, result = None , pc_queue = Queue(), cp_queue = Queue()):
        """
        ...
        #topology_metrics  = grm.graph_topology_metrics(lf,self.switch_status)
        ...
        """ reward = (-np.abs(v_err), np.mean(bcs), np.mean(cls), np.mean(ebcs), \
            topology_metrics['avg_clustering'], topology_metrics['graph_dia'],\
                topology_metrics['path_length'], topology_metrics['alg_conn']) """
        done = False
