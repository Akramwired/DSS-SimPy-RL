...
class CyberEnv(BaseEnv):
    ...
    class ForwarderDevice(SwitchPort):
        ...
        def forwarderProcess(self):
            """ This function forwards the packet i.e. receives packet from incoming channel and forwards to the out-going channel
    ...
    class ReceiverDevice(Interpreter):
        ...
        """
        def __init__(self, simpyenv,id,gymenv, rec_arrivals=True, absolute_arrivals=False, rec_waits=True, debug=True, selector=None):
        ...
        def getReward(self):
            ...
            try:
                ...
                if self.gymenv.small_Case:
                    ...
                    gthan2 = dict((k, v) for k, v in self.packets_record.items() if v >= 1)
                    if len(gthan2) == 7:
                        reward = 5
                    elif len(gthan2) == 6:
                        reward = 0
        ...
        def getDone(self):
            ...
            if self.gymenv.small_Case:
                '''
                if self.packets_rec > 5:
                    self._done = True
                '''
                gthan2 = dict((k, v) for k, v in self.packets_record.items() if v >= 10)
                if len(gthan2) == 1:
                    self._done = True
            else:
                """
    ...
    class Channel():
        """ This is the Channel class that models the communication channel.
    ...
    def __init__(self, provided_graph= None,channelModel=True, envDebug=True, R2_qlimit = 100, ch_bw=1000, with_threat= False, comp_zones = None):
    ...
    def reinitialize_network(self):
        ...
        self.senders: List[self.SenderDevice] = [
            ...
            #CyberEnv.SenderDevice(SimMan.env, 'PG1',  constArrival, constSize),
            CyberEnv.SenderDevice(SimMan.env, 'PG2', constArrival2, constSize2,debug=False)
        ]
        ...
        self.routers: List[self.ForwarderDevice] = [
            ...
            CyberEnv.ForwarderDevice(SimMan.env,'R2',rate=200.0, qlimit=0,debug=self.envDebug),
    ...
    def reset(self):
        ...
        if self.with_threat:
            self.rtr_compromised = self.threat_model(random.randint(1,1))
