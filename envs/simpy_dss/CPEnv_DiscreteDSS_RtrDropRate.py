...
import gym
# Build the discrete space variant of the Open DSS
from envs.openDSSenvSB_DiscreteSpace import openDSSenv
# Build the cyber env with router packet drop rate 
...
class CyberPhysicalEnvDummy:
    ...
    def step(self, actions):
        ...
        for env, ac in zip(self.envs, actions):
            ...
            if done:
                env.reset()
                infos_new={"terminal_observation":obs}

        return obs, rewards, all(dones), infos_new
...
class CyberPhysicalEnvMT(gym.Env):
    ...
    def __init__(self, cenv, penv, comp_zones):
        ...
        self.map_sw = {0:'Sw1',1:'Sw2',2:'Sw3',3:'Sw4',4: 'Sw5',5:'Sw6',6:'Sw7',7:'Sw8'}
        ...
        if cenv.channelModel == False:
            self.observation_space = spaces.Box(low=0, high=1000000.0, shape=(cenv.deviceCount + len(penv.critical_loads)+len(penv.line_faults),), dtype=np.float32)
            self.action_space = spaces.MultiDiscrete([cenv.deviceCount, 5, len(penv.switch_names)])
        else:
            self.observation_space = spaces.Box(low=0, high=1000000.0, shape=(len(cenv.channels) + len(penv.critical_loads)+len(penv.line_faults),), dtype=np.float32)
            self.action_space = spaces.MultiDiscrete([cenv.deviceCount, 5, len(penv.switch_names)])
...
if __name__ == '__main__':
    ...
    G = create_network2()
    cenv = CyberEnv(provided_graph=G, channelModel=False,envDebug=False, R2_qlimit=200, ch_bw = 2500,with_threat=True)
